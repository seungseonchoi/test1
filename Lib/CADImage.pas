...
{$ENDIF}
  , System.Generics.Collections //최승선 수정
  ;
...
{$ENDIF}
    ...
    procedure ScanAllEntities; // 최승선 수정
//IsgXMLObject
...
var
  ...
  EntityKeyList: TList<String>;  // 최승선 수정
...
{$ENDIF}
  ...
  // 최승선 수정: 블록(G 태그)을 올바르게 감싸도록 변경
  if Entity is TsgDXFInsert then
  begin
    if Assigned(TsgDXFInsert(Entity).BlockRecord) and (TsgDXFInsert(Entity).BlockRecord.Name <> '') then
      BlockNameStr := TsgDXFInsert(Entity).BlockRecord.Name
    else
      Exit;

    if Assigned(TsgDXFInsert(Entity).Block) and (TsgDXFInsert(Entity).Block.Count > 0) then
    begin
      if Assigned(FExpProcs) then
        FExpProcs.DoExportBlockAW('<g id="' + BlockNameStr + '">' + #13#10);
ShowMessage('블록 내부 엔티티 개수: ' + IntToStr(TsgDXFInsert(Entity).Block.Count));

      for i := 0 to TsgDXFInsert(Entity).Block.Count - 1 do
      begin
        E := CADImage.Converter.Entities[i];
        if E is TsgDXFInsert then
          ShowMessage('블록 엔티티: ' + TsgDXFInsert(E).BlockRecord.Name + ', Layer: ' + E.Layer.Name);

        ShowMessage('변환 시도: ' + IntToStr(Entity.Handle) + ' / Layer: ' + Entity.Layer.Name);

        DrawEntity(TsgDXFInsert(Entity).Block.Entities[i]);
      end;

      if Assigned(FExpProcs) then
        FExpProcs.DoExportBlockAW('</g>' + #13#10);
    end;
...
//최승선 수정
procedure TsgCADImage.ScanAllEntities;
var
  i: Integer;
  Entity: TsgDXFEntity;
begin
  if EntityKeyList = nil then
    EntityKeyList := TList<String>.Create
  else
    EntityKeyList.Clear;  // 기존 데이터 초기화
  // CAD 엔티티 개수 확인 후 저장
  if Self.Converter.Counts[csEntities] > 0 then
  begin
    for i := 0 to Self.Converter.Counts[csEntities] - 1 do
    begin
      Entity := Self.Converter.Entities[i];
      if not (Entity is TsgDXFInsert) then
      begin
        EntityKeyList.Add(UIntToStr(Entity.Handle));
      end
      else
      begin
        EntityKeyList.Add(UIntToStr(Entity.Handle));
